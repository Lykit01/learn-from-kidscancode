...
from tilemap import *
from random import choice,randrange,uniform,randint,random
...
class Mob(pg.sprite.Sprite):
    def __init__(self,game,x,y):
        ...
        self.pos=vec(x,y)
        self.vel=vec(0,0)
        self.acc=vec(0,0)#donot instantly change direction
        self.rect.center=self.pos
        self.rot=0
...
class Bullet(pg.sprite.Sprite):
    def __init__(self,game,pos,dir):
        ...
        self.hit_rect=pg.Rect(self.rect.x,self.rect.y,self.rect.width/2,self.rect.height/2)
        self.hit_rect.center=self.rect.center
        self.damage=WEAPONS[game.player.weapon]['damage']
    ...
    def update(self):
        ...
        if pg.sprite.spritecollideany(self,self.game.walls):
            self.kill()
        if pg.time.get_ticks()-self.spown_time>WEAPONS[self.game.player.weapon]['bullet_lifetime']:
            self.kill()
...
class Item(pg.sprite.Sprite):
    def __init__(self,game,pos,type):
        ...
        pg.sprite.Sprite.__init__(self,self.groups)
        self.game=game
        self.image=pg.transform.scale(game.item_images[type],(30,30))
        self.rect=self.image.get_rect()
        self.hit_rect=self.rect
        self.type=type
        self.pos=pos
